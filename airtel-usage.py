...
def main():
	...
	if m != None:
		#print m.group(1);
		p = subprocess.Popen("""/usr/bin/notify-send "airtel-usage: You are left with {0} GB this month." """.format(m.group(1)), shell=True);
		os.waitpid(p.pid, 0);
	else:
		p = subprocess.Popen("/usr/bin/notify-send Something went wrong.", shell=True);
                os.waitpid(p.pid, 0);
